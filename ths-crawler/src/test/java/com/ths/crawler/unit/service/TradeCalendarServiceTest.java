...
/**
 * TradeCalendarService 单元测试
 * <p>
 * TDD红灯阶段：先写测试用例定义行为契约，再写实现让测试通过。
 * 测试范围：交易日判定、最近交易日查询、前一交易日查询、缓存机制
 */
...
class TradeCalendarServiceTest {
    ...
    class IsTradeDayTest {
        ...
        void 数据库无记录时退化为周末判断() {
            // Mapper返回null，应退化为简单的周末判断
            LocalDate workday = LocalDate.of(2026, 6, 24); // 周三
            LocalDate saturday = LocalDate.of(2026, 6, 27); // 周六
    ...
    class GetLatestTradeDayTest {
        ...
        void 当日是交易日_返回当日() {
            LocalDate today = LocalDate.of(2026, 6, 24); // 周三
            when(tradeCalendarMapper.selectLatestTradeDayBeforeOrOn(today))
                    .thenReturn(today);
        ...
        void 当日是周末_返回上周五() {
            ...
            when(tradeCalendarMapper.selectLatestTradeDayBeforeOrOn(saturday))
                    .thenReturn(friday);
        ...
        void 当日是国庆_返回9月30日() {
            ...
            when(tradeCalendarMapper.selectLatestTradeDayBeforeOrOn(nationalDay))
                    .thenReturn(lastDayBeforeHoliday);

            assertThat(tradeCalendarService.getLatestTradeDay(nationalDay))
                    .isEqualTo(lastDayBeforeHoliday);
        }
    ...
    class GetPreviousTradeDayTest {
        ...
        @Test
        @DisplayName("正常情况：周一的前一交易日是上周五")
        void 周一的前一交易日是上周五() {
            LocalDate monday = LocalDate.of(2026, 6, 22);
            LocalDate friday = LocalDate.of(2026, 6, 19);
            when(tradeCalendarMapper.selectPreviousTradeDay(monday))
                    .thenReturn(friday);
        ...
        @Test
        @DisplayName("正常情况：周三的前一交易日是周二")
        void 周三的前一交易日是周二() {
            LocalDate wednesday = LocalDate.of(2026, 6, 24);
            LocalDate tuesday = LocalDate.of(2026, 6, 23);
            when(tradeCalendarMapper.selectPreviousTradeDay(wednesday))
                    .thenReturn(tuesday);
        ...
        void 国庆后前一交易日() {
            ...
            LocalDate lastDayBeforeHoliday = LocalDate.of(2026, 9, 30);
            when(tradeCalendarMapper.selectPreviousTradeDay(firstDayAfterHoliday))
                    .thenReturn(lastDayBeforeHoliday);

            assertThat(tradeCalendarService.getPreviousTradeDay(firstDayAfterHoliday))
                    .isEqualTo(lastDayBeforeHoliday);
        }
    }
    ...
    class GetTradeDaysTest {
        ...
        void 查询年度交易日列表() {
            List<TradeCalendarEntity> mockEntities = List.of(
                    TradeCalendarEntity.builder().tradeDate(LocalDate.of(2026, 1, 2)).isOpen(1).year(2026).build(),
                    TradeCalendarEntity.builder().tradeDate(LocalDate.of(2026, 1, 5)).isOpen(1).year(2026).build(),
                    TradeCalendarEntity.builder().tradeDate(LocalDate.of(2026, 1, 6)).isOpen(1).year(2026).build()
            );
            when(tradeCalendarMapper.selectTradeDaysByYear(2026)).thenReturn(mockEntities);
    ...
    class CacheTest {
        ...
        void 年度交易日缓存() {
            when(tradeCalendarMapper.selectTradeDaysByYear(2026))
                    .thenReturn(List.of(TradeCalendarEntity.builder().tradeDate(LocalDate.of(2026, 1, 2)).isOpen(1).year(2026).build()));
